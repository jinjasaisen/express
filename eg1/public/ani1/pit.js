...
this.onenterframe = this.damage;
            ...
            // スコア加算
            if (this.interval == SUPER_INTERVAL) {
                tm.Sound.play("hit2");
                scoreLabel.add(100);
            }
            else {
...
this.tl.delay(100);
    ...
    hide: function() {
        ...
        if (this.frame <= 0) {
            this.setWait(Math.random()*100, this.appear);
            this.changeType();
        }
    },
...
this.tl.moveBy(0, -15, 10).moveBy(0, 15, 10);

}

if (this.time > 30) {

this.tl.moveBy(0, -700, 10).moveBy(0, 700, 10);
this.tl.fadeIn(30);
this.frame = 0;
this.setWait(Math.random()*200, this.appear);
...
this.changeType();
    ...
    changeType: function() {
        if (Math.floor(Math.random()*16) == 1) {
            this.interval = SUPER_INTERVAL;
            this.image = game.assets[SUPER];
        }
        else {
            this.interval = MOGURA_INTERVAL;
            this.image = game.assets[MOGURA];
        }
    }
});
